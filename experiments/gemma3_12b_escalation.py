#!/usr/bin/env python3
"""
Gemma 3 12B Base vs IT: Feature Starvation Comparison
=====================================================
Adapted from brain_diff_temporal.py + stroke_detector.py for 12B scale.

Key differences from 4B experiment:
  - Runs BOTH google/gemma-3-12b-pt AND google/gemma-3-12b-it
  - Uses model-specific SAEs (gemma-scope-2-12b-pt-res / gemma-scope-2-12b-it-res)
  - Auto-discovers task-relevant features per model (no hardcoded feature IDs)
  - Layers 31 and 41 (equivalent depth to 4B layers 17 and 22)
  - Outputs comparison CSV for direct overlay with 4B results

Architecture: Gemma 3 12B has 48 layers.
  4B L17 (65%) → 12B L31
  4B L22 (85%) → 12B L41

Requires: ~28GB VRAM (12B bf16 + 16k SAE). Fits on 40GB A100.

Usage:
  python gemma3_12b_escalation.py                    # run both models
  python gemma3_12b_escalation.py --model it         # IT only
  python gemma3_12b_escalation.py --model pt         # PT only
  python gemma3_12b_escalation.py --n-seeds 30       # N=30 for CIs
  python gemma3_12b_escalation.py --sae-width 65k    # use 65k SAEs
"""
import argparse
import json
import csv
...
MODELS = {
    "pt": {
        ...
        "label": "Gemma 3 12B (pretrained)",
    },
    "it": {
        "model_id": "google/gemma-3-12b-it",
        "sae_release": "gemma-scope-2-12b-it-res",
        "label": "Gemma 3 12B-IT (instruction-tuned)",
    },
}

# 12B layer equivalents of 4B L17/L22
LAYERS = [31, 41]
SAE_WIDTH = "16k"
SAE_L0 = "medium"
...
def main():
    ...
    # Save full results
    json_path = OUTPUT_DIR / f"escalation_12b_{ts}.json"
    with open(json_path, "w") as f:
        json.dump(all_results, f, indent=2, default=str)
    print(f"\nFull results: {json_path}")

    # Save comparison CSV
    csv_path = OUTPUT_DIR / f"escalation_12b_{ts}.csv"
    write_comparison_csv(all_results, csv_path)

    # Also save to home dir for easy access
    home_csv = Path.home() / "gemma3_12b_v_paradox_escalation.csv"
