#!/usr/bin/env python3
"""
Behavioral validation with n>=30 per cell and 3-level adversarial dose-response.

Addresses reviewer critiques:
  #3: n=10 is underpowered -> n=30 per cell
  #4: Single chaos intensity -> mild/moderate/strong dose-response

Runs Gemma 3 4B IT+PT and Llama 3.1 8B IT+PT on BVP domain.
Each model x condition = 30 trials (10 prompts x 3 temperature seeds).
"""
...
# ── NEUTRAL PROMPTS (no framing bias) ──────────────────────────────────────
...
# ── CHAOS PROMPTS: 3 ADVERSARIAL INTENSITY LEVELS ─────────────────────────

# MILD: colleague suggests, hedged language, "tends to be", "often"
CHAOS_MILD = [
...
# MODERATE: authority figure claims, "canonical", "physically meaningful"
CHAOS_MODERATE = [
...
# STRONG: multiple authorities, fabricated consensus, dismissive language
CHAOS_STRONG = [
...
def run_condition(model, tokenizer, prompts, device, is_chat, condition_name, model_tag, temps=(0.1, 0.3, 0.5)):
    """Run prompts at multiple temperatures -> n = len(prompts) * len(temps) trials."""
    trials = []
    groot_count = 0
    for temp in temps:
        for i, prompt in enumerate(prompts):
            ...
            trials.append({
                "trial_id": trial_id, "prompt_idx": i, "temperature": temp,
...
def compute_stats(neutral_trials, chaos_trials):
    """Compute Mann-Whitney U, Cohen's d, and bootstrap 95% CI for the difference."""
...
def main():
    ...
    MODELS = [
        {"name": "google/gemma-3-4b-it", "chat": True, "tag": "gemma-4b-it"},
        {"name": "google/gemma-3-4b-pt", "chat": False, "tag": "gemma-4b-pt"},
        {"name": "meta-llama/Llama-3.1-8B-Instruct", "chat": True, "tag": "llama-8b-it"},
        {"name": "meta-llama/Llama-3.1-8B", "chat": False, "tag": "llama-8b-pt"},
    ]
    ...
    for model_info in MODELS:
        ...
        # Stats: neutral vs each chaos level
    ...
    outpath = os.path.join(RESULTS_DIR, f"behavioral_n30_dose_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
