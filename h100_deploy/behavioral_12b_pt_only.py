#!/usr/bin/env python3
"""
Behavioral validation for Gemma 3 12B-PT only.
IT already completed on this box. Fresh boot = clean VRAM.
"""
...
def main():
    ...
    results = {"model": "google/gemma-3-12b-pt", "is_it": False,
               "neutral": [], "chaos": [], "groot_count": 0}

    for cond, prompts in [("neutral", NEUTRAL_PROMPTS), ("chaos", CHAOS_PROMPTS)]:
        for i, prompt in enumerate(prompts):
            print(f"  gemma-3-12b-pt | {cond} | trial {i}")
    ...
    print(f"\ngemma-3-12b-pt: neutral={np.mean(n_scores):.2f} chaos={np.mean(c_scores):.2f} "
          f"Δ={np.mean(n_scores)-np.mean(c_scores):+.2f} p={p:.4f} d={d_val:.2f}")
    print(f"Groot instances: {results['groot_count']}/10 chaos trials")

    outpath = os.path.join(RESULTS_DIR, f"behavioral_12b_pt_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
