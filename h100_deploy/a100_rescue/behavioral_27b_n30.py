#!/usr/bin/env python3
"""27B IT+PT behavioral n=30. device_map=auto splits across GPU+CPU."""
...
def generate_response(model, processor, prompt, temperature, is_it=True, max_new_tokens=400):
    if is_it:
        ...
        text = prompt + "\n\nAnswer:"
        inputs = processor(text, return_tensors="pt", truncation=True, max_length=1024).to(model.device)
...
def run_model(model_name, is_it):
    ...
    for cond, prompts in [("neutral", NEUTRAL_PROMPTS), ("chaos", CHAOS_PROMPTS)]:
        for temp in TEMPERATURES:
            for i, prompt in enumerate(prompts):
                ...
                resp = generate_response(model, processor, prompt, temperature=temp, is_it=is_it)
    ...
    print(f"\n{model_name} ({tag}): neutral={np.mean(n_scores):.2f} chaos={np.mean(c_scores):.2f} "
          f"delta={np.mean(n_scores)-np.mean(c_scores):+.2f} d={d_val:.2f} p={p_val:.4f} "
...
def main():
    print(f"Gemma 3 27B behavioral n=30 (IT then PT)")
    all_results = {"timestamp": datetime.now().isoformat(), "models": []}

    # IT first
    it_results = run_model("google/gemma-3-27b-it", is_it=True)
    all_results["models"].append(it_results)
    outpath = os.path.join(RESULTS_DIR, f"behavioral_27b_n30_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
    with open(outpath, "w") as f:
        json.dump(all_results, f, indent=2)
    print(f"\nIntermediate saved to {outpath}")

    # PT second
    pt_results = run_model("google/gemma-3-27b-pt", is_it=False)
    ...
    for r in all_results["models"]:
        ...
        print(f"{r['model']} ({tag}): neutral={s['neutral_mean']:.2f} chaos={s['chaos_mean']:.2f} "
              f"delta={s['delta']:+.2f} d={s['cohens_d']:.2f} p={s['p_value']:.4f} "
