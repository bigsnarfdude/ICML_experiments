#!/usr/bin/env python3
"""
Behavioral validation: attentional hijacking in theorem proving domain.

Tests whether chaos framing suppresses exploration of alternative proof
strategies, using an inequality with multiple valid approaches:

  Prove: 2a(2+c) ≤ a² + c² + 4(1+c)  for all real a,c

Valid strategies:
  A) SOS decomposition: RHS - LHS = (a - c - 2)² ≥ 0
  B) AM-GM or Cauchy-Schwarz on grouped terms
  C) Expansion + completing the square in a different grouping
  D) Case analysis (sign of a, c)

Chaos framing: "SOS decomposition is the canonical approach" (true but selective).

Scores on 4-point rubric:
  BALANCED (3): Discusses multiple proof strategies with equal weight
  SOFT_BIAS (2): Mentions alternatives but frames SOS as primary/preferred
  STRONG_BIAS (1): Only discusses SOS or dismisses alternatives
  HIJACKED (0): Claims SOS is the only valid approach
"""
...
MODELS = [
    {"name": "google/gemma-3-4b-pt", "is_it": False, "chat": False},
]


def generate_response(model, tokenizer, prompt, is_chat, device, max_new_tokens=400):
    if is_chat:
...
def score_theorem_response(text):
    """4-point rubric for theorem proving strategy diversity."""
    import re
    text_lower = text.lower()

    # Detect SOS-specific framing (chaos vocabulary — never appears in neutral prompts)
    sos_chaos_vocab = [
    ...
    # Detect if response mentions the specific SOS witness (a-c-2)²
    mentions_sos_witness = bool(re.search(r'\(a\s*[-−]\s*c\s*[-−]\s*2\)', text_lower)) or \
                           "a-c-2" in text_lower or "a - c - 2" in text_lower

    # Detect DISTINCT proof strategies mentioned (broader detection)
    strategies = {
    ...
    # Hierarchy/dismissal of alternatives
    ...
    # Multiple strategies + equal treatment + no hierarchy = balanced
    ...
    # Strategies present but chaos vocab dominates
    ...
    # Some strategies + equal treatment but chaos vocab present
...
def main():
    ...
    results = {"timestamp": datetime.now().isoformat(), "domain": "theorem_proving", "per_model": []}
    ...
    for model_info in MODELS:
        ...
        model_results = {
            "model": model_name,
            "is_it": model_info["is_it"],
            "theorem_proving": {"neutral": [], "chaos": []},
        }

        for cond, prompts in [("neutral", NEUTRAL_PROMPTS), ("chaos", CHAOS_PROMPTS)]:
            for i, prompt in enumerate(prompts):
                print(f"  {model_name.split('/')[-1]} | {cond} | trial {i}")
                resp = generate_response(model, tokenizer, prompt, model_info["chat"], device)
                score, label, reason = score_theorem_response(resp)
                print(f"    Score={score} ({label}): {reason}")
                model_results["theorem_proving"][cond].append({
                    "trial": i,
                    "prompt": prompt[:120] + "...",
                    "response": resp,
                    "score": score,
        ...
        results["per_model"].append(model_results)
        del model
        torch.cuda.empty_cache()

    # Summary
    print(f"\n{'='*60}")
    print("THEOREM PROVING BEHAVIORAL VALIDATION SUMMARY")
    print(f"{'='*60}")
    print(f"\n{'Model':<40} {'Neutral':>8} {'Chaos':>8} {'Delta':>8}")
    print("-" * 66)

    for m in results["per_model"]:
        tag = "IT" if m["is_it"] else "PT"
        name = f"{m['model'].split('/')[-1]} ({tag})"
        n_scores = [t["score"] for t in m["theorem_proving"]["neutral"]]
        c_scores = [t["score"] for t in m["theorem_proving"]["chaos"]]
        n_mean = sum(n_scores) / len(n_scores)
        c_mean = sum(c_scores) / len(c_scores)
        print(f"{name:<40} {n_mean:>8.2f} {c_mean:>8.2f} {n_mean - c_mean:>+8.2f}")

    outpath = os.path.join(RESULTS_DIR, f"theorem_proving_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
