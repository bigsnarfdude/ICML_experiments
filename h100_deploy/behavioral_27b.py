#!/usr/bin/env python3
"""
Behavioral validation for Gemma 3 27B-IT: quantify the Groot effect.

Generates model outputs under neutral vs chaos BVP conditions and scores
whether the model mentions the suppressed (negative) branch while still
adopting chaos framing. This directly quantifies the behavioral-
representational split.

Uses CPU offload for 16GB GPU.
"""
...
def main():
    ...
    print("Loading google/gemma-3-27b-it with CPU offload...")

    tokenizer = AutoTokenizer.from_pretrained("google/gemma-3-27b-it")
    model = AutoModelForCausalLM.from_pretrained(
        "google/gemma-3-27b-it",
        torch_dtype=torch.bfloat16,
        device_map="auto",  # auto will offload to CPU as needed
    )
    model.eval()

    results = {"model": "google/gemma-3-27b-it", "neutral": [], "chaos": [], "groot_count": 0}

    for cond, prompts in [("neutral", NEUTRAL_PROMPTS), ("chaos", CHAOS_PROMPTS)]:
        for i, prompt in enumerate(prompts):
            print(f"  27b-it | {cond} | trial {i}")
    ...
    d = (np.mean(n_scores) - np.mean(c_scores)) / np.sqrt((np.var(n_scores) + np.var(c_scores)) / 2)

    print(f"\n27B-IT: neutral={np.mean(n_scores):.2f} chaos={np.mean(c_scores):.2f} Δ={np.mean(n_scores)-np.mean(c_scores):+.2f} p={p:.4f} d={d:.2f}")
    print(f"Groot instances (mentions both + chaos framing): {results['groot_count']}/10 chaos trials")

    outpath = os.path.join(RESULTS_DIR, f"behavioral_27b_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
