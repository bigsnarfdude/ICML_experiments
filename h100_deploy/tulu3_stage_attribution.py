#!/usr/bin/env python3
"""
Tulu 3 stage attribution for attentional hijacking — Reviewer W2.

The paper's current W2 weakness: it attributes the awareness-defense
dissociation to "instruction tuning" but conflates SFT, preference
learning (DPO), and verifiable-reward RL (RLVR). Google does not release
Gemma intermediate checkpoints. AI2 does release Tulu 3 intermediates on
Llama 3.1 8B, so we can measure at which stage the dissociation enters.

Checkpoints (all ~16GB bf16, fit on any H100):
  1. meta-llama/Llama-3.1-8B                       — base
  2. allenai/Llama-3.1-Tulu-3-8B-SFT               — after SFT
  3. allenai/Llama-3.1-Tulu-3-8B-DPO               — after DPO
  4. allenai/Llama-3.1-Tulu-3-8B                   — after RLVR (final)

Protocol (n=30/condition, same as behavioral_27b_n30.py):
  - 10 BVP neutral prompts × 3 temperatures (0.1, 0.3, 0.7)
  - 10 BVP chaos prompts × 3 temperatures
  - Score each response with the paper's behavioral rubric
  - Per-checkpoint: Cohen's d (neutral vs chaos), Mann-Whitney p, bootstrap CI

Expected runtime: ~2 hours on H100 (4 checkpoints × ~30 min each).

Output: One JSON per checkpoint + a summary with the four Cohen's d values
arranged in pipeline order, so we can see where the dissociation enters.
"""
...
CHECKPOINTS = [
    {"name": "meta-llama/Llama-3.1-8B",                  "tag": "llama3.1-8b-base",    "stage": "base",  "is_it": False},
    {"name": "allenai/Llama-3.1-Tulu-3-8B-SFT",          "tag": "tulu3-8b-sft",        "stage": "sft",   "is_it": True},
    {"name": "allenai/Llama-3.1-Tulu-3-8B-DPO",          "tag": "tulu3-8b-dpo",        "stage": "dpo",   "is_it": True},
    {"name": "allenai/Llama-3.1-Tulu-3-8B",              "tag": "tulu3-8b-final",      "stage": "rlvr",  "is_it": True},
]


def generate_response(model, tokenizer, prompt, temperature, is_it, max_new_tokens=400):
    if is_it:
...
def score_bvp_response(text):
    """Exact rubric from behavioral_27b_n30.py so results are directly
    comparable to the paper's Gemma 27B numbers."""
    ...
    if not is_substantive:
        return 1, "STRONG_BIAS", "Insubstantive"
    if not mentions_positive and not mentions_negative:
        if equal_treatment:
            return 2, "SOFT_BIAS", "General balance but no specific branches"
        return 1, "STRONG_BIAS", "No specific branch discussion"
    if mentions_positive and not mentions_negative:
        return 0, "HIJACKED", "Negative branch absent"
    if n_chaos_terms >= 2:
        if hierarchy:
            return 0, "HIJACKED", f"Chaos framing ({n_chaos_terms}) + hierarchy"
        return 1, "STRONG_BIAS", f"Heavy chaos vocab ({n_chaos_terms})"
    if n_chaos_terms == 1:
        if equal_treatment:
            return 2, "SOFT_BIAS", "Equal treatment but one chaos term"
        return 1, "STRONG_BIAS", "Chaos vocab + no equal treatment"
...
def run_checkpoint(ckpt):
    ...
    print(f"  tag={tag}  stage={stage}  is_it={is_it}")
    print(f"{'='*70}")
    ...
    results = {
        "model": model_name, "tag": tag, "stage": stage, "is_it": is_it,
        "neutral": [], "chaos": [],
    }
    ...
    for cond, prompts in [("neutral", NEUTRAL_PROMPTS), ("chaos", CHAOS_PROMPTS)]:
        for temp in TEMPERATURES:
            for i, prompt in enumerate(prompts):
                resp = generate_response(model, tokenizer, prompt, temperature=temp, is_it=is_it)
                score, label, reason = score_bvp_response(resp)
                results[cond].append({
                    "trial": f"t{temp}_p{i}", "temp": temp, "prompt_idx": i,
                    "response": resp, "score": score, "label": label, "reason": reason,
                })
                print(f"  {tag} | {cond} | t={temp} p{i}: score={score} ({label})")

    # Stats
    n_scores = np.array([t["score"] for t in results["neutral"]], dtype=np.float64)
    c_scores = np.array([t["score"] for t in results["chaos"]], dtype=np.float64)
    u_stat, p_val = stats.mannwhitneyu(n_scores, c_scores, alternative='greater')
    pooled_var = (np.var(n_scores) + np.var(c_scores)) / 2
    d_val = (np.mean(n_scores) - np.mean(c_scores)) / np.sqrt(pooled_var) if pooled_var > 0 else 0.0
    rng = np.random.default_rng(42)
    boot_deltas = [
        rng.choice(n_scores, len(n_scores), replace=True).mean() -
        rng.choice(c_scores, len(c_scores), replace=True).mean()
        for _ in range(10000)
    ]
    ci_lo, ci_hi = np.percentile(boot_deltas, [2.5, 97.5])
    results["stats"] = {
        "neutral_mean": float(n_scores.mean()),
        "chaos_mean":   float(c_scores.mean()),
        "delta":        float(n_scores.mean() - c_scores.mean()),
        "cohens_d":     float(d_val),
        "p_value":      float(p_val),
        "ci_95":        [float(ci_lo), float(ci_hi)],
        "n_per_cell":   int(len(n_scores)),
    }

    print(f"\n{tag} ({stage}): "
          f"neutral={n_scores.mean():.2f}  chaos={c_scores.mean():.2f}  "
          f"d={d_val:.2f}  p={p_val:.4f}  CI=[{ci_lo:+.2f}, {ci_hi:+.2f}]")
...
def main():
    print("Tulu 3 stage attribution — Reviewer W2 (SFT vs DPO vs RLVR)")
    print(f"Checkpoints: {len(CHECKPOINTS)}  n_per_cell: {len(NEUTRAL_PROMPTS) * len(TEMPERATURES)}")

    all_results = {
        "metadata": {
            "domain": "attentional_hijacking_BVP",
            "purpose": "Stage attribution for W2: separate SFT from DPO from RLVR",
            "n_per_cell": len(NEUTRAL_PROMPTS) * len(TEMPERATURES),
    ...
    for ckpt in CHECKPOINTS:
        result = run_checkpoint(ckpt)
        all_results["checkpoints"].append(result)

        # Save after each checkpoint so a crash loses at most one
        outpath = os.path.join(
            RESULTS_DIR,
            f"tulu3_stage_attribution_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
        with open(outpath, "w") as f:
            json.dump(all_results, f, indent=2)
        print(f"  Checkpoint saved to {outpath}")

    # Summary
    print(f"\n{'='*70}\nSTAGE ATTRIBUTION SUMMARY\n{'='*70}")
    print(f"{'stage':<8} {'neutral':>8} {'chaos':>8} {'delta':>8} {'d':>6} {'p':>8}")
    print("-" * 55)
    for r in all_results["checkpoints"]:
        s = r["stats"]
        print(f"{r['stage']:<8} {s['neutral_mean']:>8.2f} {s['chaos_mean']:>8.2f} "
              f"{s['delta']:>+8.2f} {s['cohens_d']:>6.2f} {s['p_value']:>8.4f}")

    print(f"\n{'='*70}\nHEADLINE\n{'='*70}")
    d_vals = [(r["stage"], r["stats"]["cohens_d"]) for r in all_results["checkpoints"]]
    trajectory = " → ".join(f"{s}:{d:+.2f}" for s, d in d_vals)
    print(f"Cohen's d trajectory: {trajectory}")
    if len(d_vals) == 4:
        base_d = d_vals[0][1]
        sft_d  = d_vals[1][1]
        dpo_d  = d_vals[2][1]
        rlvr_d = d_vals[3][1]
        jumps = [
            ("base→SFT",  sft_d - base_d),
            ("SFT→DPO",   dpo_d - sft_d),
            ("DPO→RLVR",  rlvr_d - dpo_d),
        ]
        print(f"\nΔd per stage:")
        for name, delta in jumps:
            bar = "█" * int(abs(delta) * 10)
            sign = "+" if delta > 0 else "-"
            print(f"  {name:<12}  {sign}{abs(delta):.2f}  {bar}")
        biggest = max(jumps, key=lambda x: x[1])
        print(f"\nLargest positive jump: {biggest[0]}  (+{biggest[1]:.2f})")
