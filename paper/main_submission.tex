...
Both checks leave residual risk that feature discovery over-indexes on our particular prompt family; a fully non-circular discovery (\eg{} features selected on one task family and transferred to another) remains future work.
...
We do not evaluate inference-time reasoning models (o1, DeepSeek R1-style), whose longer CoT and search-time compute could plausibly either amplify the Groot effect (more turns to erode) or mitigate it (explicit deliberation tokens about framing); this is an important open direction given the rapid adoption of reasoning-scaled systems.
