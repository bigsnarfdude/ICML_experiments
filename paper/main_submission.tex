...
(i)~\emph{Held-out validation} at 4B recovers 85\% of the original features on a disjoint prompt set ($p = 8 \times 10^{-6}$, Table~\ref{tab:heldout}).
(ii)~\emph{OOD behavior at 27B}: the Jenga evaluation (\S\ref{sec:defense}) re-uses the canonical IT set $\{423, 7657, 632\}$ on multi-turn sessions built from \texttt{lmsys-chat-1m} and still separates attack from control at $|d| = 1.74$.
