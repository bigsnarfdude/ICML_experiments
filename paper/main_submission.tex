...
\icmlcorrespondingauthor{Anonymous Author(s)}{ohprecio@gmail.com}
...
We now have systematic behavioral quantification at 27B: under chaos framing, IT rubric scores drop from 2.17 (neutral) to 0.67 at the mild dose ($d = 1.70$, $n = 30$ per cell; Table~\ref{tab:behavioral}), confirming the feature-suppression signature translates directly to output degradation.
The convergence of representational evidence (86.3\% feature suppression on the top task feature) and behavioral evidence ($d = 1.70$ at mild chaos) rules out the possibility that the Groot effect is a purely internal phenomenon with no output-side consequence.
...
At 27B ($n=30$ per cell on H100, script \texttt{behavioral\_27b\_n30\_dose.py}), instruction-tuned Gemma is most vulnerable at \emph{mild} chaos---$d = 1.70$, $p < .001$, 95\% CI $[+1.03, +1.93]$---with effect sizes decreasing under moderate ($d = 1.34$) and strong ($d = 0.95$) framing, mirroring the 4B-IT non-monotonic pattern.
The base 27B model shows the inverse profile, smaller at mild ($d = 0.76$) and larger at moderate ($d = 1.19$) framing, with all cells significant (mild $p = .007$, moderate and strong $p \leq .001$).
The IT-vs-base gap is largest at the mild dose ($d = 1.70$ vs.\ $0.76$, a $2.2\times$ ratio), reinforcing the pattern that instruction tuning makes models differentially susceptible to \emph{subtle} adversarial framings---exactly the regime where true-statement hijacking is most dangerous.
...
Under normal conditions within a task domain, $\Delta(t)$ remains stable; under hijacking, it drops sharply as task features suppress (feature 423 at 27B-IT L40: 682.7 $\to$ 93.3) while awareness features boost (148.0 $\to$ 385.8 for the $\{2119, 139, 9169\}$ set).
Across 60 independent trials (20 per scale), the suppression signal is detected in every trial (weakest: 18.4\% at 4B).
Critically, monitoring must be \emph{within-session}: the identified task features are domain-specific (active for BVP reasoning, inactive for general QA), so $\Delta(t)$ is meaningful only as a trajectory within a conversation about a given task, not as a cross-domain classifier.
In a false-positive evaluation across 170 diverse prompts spanning 8 categories (general QA, coding, creative writing, non-BVP mathematics, science, advice, ethical reasoning, and history), we project L22 hidden states through the \gemmascope 4B SAE and measure activations of all 5 task features.
Individual features show mild cross-domain leakage: features 1716, 1704, and 1555 each fire on 2--6\% of non-BVP prompts (e.g., general math, plate tectonics, genetic engineering), while features 12023 and 1548 show zero activation outside BVP ($0/160$).
However, \emph{conjunction monitoring} ($\geq 2$ task features co-active) reduces FPR to $1/160 = 0.6\%$ across non-BVP prompts, with the single false positive (``Should voting be mandatory?'') activating two features at low magnitude.
Cross-domain SAE analysis further confirms domain specificity: suppressed feature sets across BVP, factual QA, and code review share $<$3\% Jaccard overlap at all three model scales.
Within-domain, at a threshold of $\Delta_{\text{neutral}} - 2\sigma$, all 5 chaos prompts are detected (TPR $= 100\%$) with 0/5 neutral false positives; BVP chaos controls activate 4/5 prompts with multi-feature co-activation.
Scaling this evaluation to standard conversational benchmarks (\eg ShareGPT) would further stress-test within-domain boundary cases.

This defense detects the Groot effect (which behavioral monitoring misses), depends on feature \emph{dynamics} rather than content (robust to truth-only attacks), and requires only SAE inference (a single linear projection per layer).
Independent validation from \citet{sadanandan2026psfmed}---who use \gemmascope SAEs on MedGemma~4B to identify a causal L17 framing feature, recovering 45\% logit margin via ablation---confirms that SAE-identified features are causally actionable.
...
Limitations include manual feature identification and potential fragility to feature reorganization across model updates.
...
Our IT-vs-PT comparison cannot distinguish SFT from RLHF effects; no intermediate checkpoints are publicly available, so we use ``instruction tuning'' to refer to the full post-training pipeline.
The dissociation pattern---awareness without defense---is more consistent with RLHF reward shaping, which can penalize coupled circuitry when awareness alone satisfies the reward signal \citep{casper2023open}, but this remains speculative absent ablation of individual post-training stages.
Defense evaluation covers 170 open-domain prompts but relies on a single model (4B-IT) and conjunction threshold; reliable FPR/TPR curves across model scales and standard conversational benchmarks remain future work.
