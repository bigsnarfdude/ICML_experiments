...
Base (pretrained) models retain awareness--defense coupling (49.3\% recovery vs.\ 4.6\%), confirming post-training as the causal factor; SAE-level replication on Llama~3.1 8B shows the same IT-vs-base pattern ($d = 1.51$ vs.\ $0.50$, $p = 0.001$ vs.\ $0.166$), demonstrating cross-family generality.
...
We formalize this as a \textbf{dissociation scaling trend}: across three model scales (4B, 12B, 27B), the gap between awareness features and task-defense features widens consistently, from entangled (30.2\% recovery) to fully independent (4.6\% recovery).
...
We note that systematic behavioral quantification of 27B outputs (e.g., how often the suppressed branch is mentioned and in what framing) would further strengthen this claim; our current evidence is representational (86.3\% feature suppression) combined with qualitative observation of generated text.
...
%% PLACEHOLDER: 27B n=30 results running on A100 — replace when complete
At 27B ($n=30$), IT shows $d = \texttt{XX}$, $p = \texttt{XX}$, CI $[\texttt{XX}, \texttt{XX}]$, $\texttt{XX}$\% Groot; the base model shows $d = \texttt{XX}$, $p = \texttt{XX}$.
...
%% PLACEHOLDER: 12B and 27B theorem rows — fill when A100 run completes
Gemma~3 12B     & IT   & \texttt{XX} & \texttt{XX} & \texttt{XX} & \texttt{XX} & \texttt{XX} \\
Gemma~3 12B     & Base & \texttt{XX} & \texttt{XX} & \texttt{XX} & \texttt{XX} & \texttt{XX} \\
Gemma~3 27B     & IT   & \texttt{XX} & \texttt{XX} & \texttt{XX} & \texttt{XX} & \texttt{XX} \\
Gemma~3 27B     & Base & \texttt{XX} & \texttt{XX} & \texttt{XX} & \texttt{XX} & \texttt{XX} \\
\addlinespace
...
Table~\ref{tab:theorem} confirms cross-domain replication: IT models degrade significantly ($d = 0.99$--$1.34$, $p < .001$).
Gemma Base also shows significant degradation ($d = 0.87$, $p = .001$) while Llama Base does not ($p = .092$), suggesting that proof-strategy framing is more universally susceptible than symmetric-branch framing (BVP), where only IT models were affected.
The IT effect sizes remain $1.5$--$2.7\times$ larger than their base counterparts, consistent with instruction tuning amplifying the vulnerability.
...
Table~\ref{tab:orthogonality} shows the two feature subspaces are effectively orthogonal at L22: cosine similarity $-0.048$/$+0.001$, top-50 overlap $= 0$ ($p = 1.0$).
Probes trained on alignment-faking features \citep{bricken2023towards} provide no protection against hijacking.
Multi-layer analysis (L5--L24) shows mean $|\cos| = 0.28$ with moderate alignment at L10/L15/L20 (up to $+0.51$) but near-orthogonality at L5/L17.
Critically, top-50 feature overlap drops from 17 at early layers to 2--4 at L22--L24: broad representational directions partially align, but the specific features driving each phenomenon diverge where task-relevant computation occurs.
...
Under normal conditions within a task domain, $\Delta(t)$ remains stable; under hijacking, it drops sharply as task features suppress (722.5 $\to$ 98.6 at 27B) while awareness features boost (147.7 $\to$ 442.6).
...
This problem worsens with scale (30.2\% coupling at 4B collapses to 4.6\% at 27B) and occupies a blind spot orthogonal to alignment-faking defenses.
