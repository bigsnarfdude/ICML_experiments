...
To test whether the dissociation is specific to \gemma or reflects a general consequence of instruction tuning, we replicate the SAE-level analysis on Llama~3.1 8B using EleutherAI's 131K-feature sparse autoencoders \citep{gao2024scaling}.
We compare Llama~3.1 8B-Instruct (IT) against Llama~3.1 8B (base/PT) on the Nirenberg BVP domain using the same prompt structure, with 10 prompt variants (5 discovery, 5 held-out test) and SAEs at MLP layers 23 and 29.
...
Table~\ref{tab:llama_sae} shows the pattern replicates: while both IT and PT models exhibit feature suppression under chaos framing, the IT model shows consistently larger effect sizes.
At layer 29, the IT model achieves $d = 1.61$ ($p = 0.033$) versus $d = 1.10$ ($p = 0.092$) for the base model---a 46\% larger effect.
Critically, the IT model's suppression reaches statistical significance ($p < 0.05$) while the base model's does not, consistent with instruction tuning amplifying the dissociation between awareness and defense.
This cross-family replication on a fundamentally different architecture (Llama's grouped-query attention vs.\ \gemma's multi-query attention) with independently trained SAEs provides evidence that the dissociation is a general consequence of current post-training methods, not an artifact of the \gemma family.
...
We replicate the core IT-vs-PT dissociation on Llama~3.1 8B using EleutherAI's 131K-feature SAEs (\S\ref{sec:llama_replication}), finding consistent patterns ($d = 1.61$ IT vs.\ $d = 1.10$ PT at layer 29).
However, the full dissociation \emph{scaling} trend (increasing decoupling with model size) has been verified only within the \gemma family, and the wide variance in post-training methodologies means that the scaling dynamics we observe may not generalize to all RLHF implementations.
