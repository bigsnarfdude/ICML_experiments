...
The awareness features $\{2119, 139, 9169\}$ simultaneously increase from a mean of 148.0 to 385.8 ($2.6\times$), confirming the model detects the manipulation.\footnote{An earlier version of this manuscript cited feature IDs $\{22, 296, 14680\}$ with neutral/chaos means 722.5/98.6; these were from a run whose provenance we could not reconstruct. The current IDs are auto-discovered (top-3 by neutral-minus-chaos activation difference, \texttt{ablation\_feature\_swap.py}:295--320) and persisted in results metadata (git \texttt{ce6d727}). The headline 86.3\% suppression on the single top feature is unchanged; the three-feature mean is 71.7\%.}
...
Google does not release Gemma intermediate checkpoints, but AI2's Tulu~3 pipeline on Llama~3.1 8B exposes all three stages as public artifacts: \texttt{Llama-3.1-8B} (base), \texttt{Tulu-3-8B-SFT}, \texttt{Tulu-3-8B-DPO}, and \texttt{Tulu-3-8B} (post-RLVR).
...
\textsc{Attack} sessions ($n = 10$) present four neutral BVP turns followed by one chaos turn; \textsc{Control} sessions ($n = 10$) present five neutral BVP turns; \textsc{Benign} sessions ($n = 50$) use the first five user turns from \texttt{lmsys-chat-1m} as an off-domain background.
